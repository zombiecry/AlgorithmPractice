...
class SimilarRatingGraph
        ...
        public: 
			vector<int> d;
			vector<int> r;
			bool   dpf[MAX_N+1][MAX_N+1];
			double dp[MAX_N+1][MAX_N+1];
			double Solve(int p,int q){
				if (p==0 || q==0) return 0;
				if(dpf[p][q]){ return dp[p][q];}
				double res=0;
				res=max(res,Solve(p-1,q));
				res=max(res,Solve(p,q-1));
				if (p==q) {
					dpf[p][q]=true;
					dp[p][q]=res;
					return res;
				...
				int l=0;
				int fdd1,fdd2;
				while(p-1-l>=0 && q-1-l>=0){
					if (l==1){
						fdd1=d[p-2]-d[p-1];
						fdd2=d[q-2]-d[q-1];
					}
					if(l>=1){
						int dd1=d[p-1-l]-d[p-l];
						int dd2=d[q-1-l]-d[q-l];
						int dr1=r[p-1-l]-r[p-l];
						int dr2=r[q-1-l]-r[q-l];
						if (dd1*fdd2!=dd2*fdd1) break;
						if (dr1*fdd2!=dr2*fdd1) break;
					}
					l++;
				}
				double len=0;
				for(int i=1;i<l;i++){
					int prev=p-i;
					int cur=p-i-1;
					double d1=abs(r[prev]-r[cur]);
					double d2=abs(d[prev]-d[cur]);
					len+=sqrt(double(d1*d1+d2*d2));
				}
				res=max(res,len);
				res=max(res,Solve(p-l,q-l));
				if (len>1){
					int aaa=1;
				}
				dpf[p][q]=true;
...
// BEGIN CUT HERE
	...
	void test_case_0() { 
		int Arr0[] = {1, 500000, 999999};
		vector <int> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0])));
		int Arr1[] = {42, 42, 42};
		vector <int> Arg1(Arr1, Arr1 + (sizeof(Arr1) / sizeof(Arr1[0])));
		double Arg2 = 499999.0;
		verify_case(0, Arg2, maxLength(Arg0, Arg1));
	}
// END CUT HERE
